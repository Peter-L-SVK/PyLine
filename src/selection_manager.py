# ----------------------------------------------------------------
# PyLine 0.9.8 - Selection Manager (GPLv3)
...
class SelectionManager(BaseManager):
    ...
    def __init__(self, hook_utils: Any) -> None:
    ...
    def end_selection(self, line_number: int, filename: Optional[str] = None) -> None:
    ...
    def get_selected_text(self, lines: List[str], filename: Optional[str] = None) -> str:
        ...
        if selection_result and "text" in selection_result:
            return str(selection_result["text"])
