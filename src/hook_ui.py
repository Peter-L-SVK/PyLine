...
class HookManagerUI:
    ...
    def list_all_hooks(self, detailed=False):
        ...
        for category, category_hooks in hooks_by_category.items():
            ...
            for hook_type, type_hooks in hooks_by_type.items():
                ...
                for hook in sorted(type_hooks, key=lambda x: x['name']):
                    ...
                    if detailed:
                        description = self._get_hook_info(Path(hook['path'])).get('description', 'No description')
                        print(f"      Description: {description}")
                        print(f"      Path: {hook['path']}")
                    print()
    
    def toggle_hook(self, hook_id, enable=True):
    ...
    def find_hook(self, search_term):
        ...
        for hook in self.hook_mgr.list_all_hooks():
            ...
            if (search_lower in hook['name'].lower() or 
                search_lower in hook['id'].lower() or
                search_lower in hook_info.get('description', '').lower()):
                found_hooks.append({
                    **hook,
    ...
    def _get_hook_info(self, hook_file):
    ...
    def get_hook_display_name(self, hook_id):
